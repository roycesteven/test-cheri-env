...
compartment("heap-buffer-over-read")
    ...
    add_files("heap-buffer-over-read.c")

-- Firmware image for the example.
firmware("heap-buffer-over-read-vulnerability")
    add_deps("stdio")
    add_deps("heap-buffer-over-read")
...
on_run(function (target)
    ...
    local rundir = "../../submit-cheriot/exercises-c/01.heap-buffer-over-read/runs/run_" .. os.date("%Y%m%d_%H%M%S")
    os.mkdir(rundir)
    ...
    if get_config("debug_run") == true then
        ...
        local log_file = "../../submit-cheriot/exercises-c/01.heap-buffer-over-read/debug"
