...
compartment("heap-buffer-over-write")
    -- memcpy
    add_deps("freestanding", "debug", "unwind_error_handler")
    add_files("heap-buffer-over-write.cc")

-- Firmware image for the example.
firmware("heap-buffer-over-write-vulnerability")
    add_deps("freestanding", "debug", "stdio", "heap-buffer-over-write")
...
on_run(function (target)
    ...
    local rundir = "../../submit-cheriot/exercises-c++/02.heap-buffer-over-write/runs/run_" .. os.date("%Y%m%d_%H%M%S")
    os.mkdir(rundir)
    ...
    if get_config("debug_run") == true then
        ...
        local log_file = "../../submit-cheriot/exercises-c++/02.heap-buffer-over-write/debug"
