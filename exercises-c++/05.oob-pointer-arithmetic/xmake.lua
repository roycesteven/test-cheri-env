...
compartment("oob-pointer-arithmetic")
    -- memcpy
    add_deps("freestanding", "debug", "unwind_error_handler")
    add_files("oob-pointer-arithmetic.cc")

-- Firmware image for the example.
firmware("oob-pointer-arithmetic-vulnerability")
    add_deps("freestanding", "debug", "stdio", "oob-pointer-arithmetic")
...
on_run(function (target)
    ...
    local rundir = "../../submit-cheriot/exercises-c++/05.oob-pointer-arithmetic/runs/run_" .. os.date("%Y%m%d_%H%M%S")
    os.mkdir(rundir)
    ...
    if get_config("debug_run") == true then
        ...
        local log_file = "../../submit-cheriot/exercises-c++/05.oob-pointer-arithmetic/debug"
